function accuracy
  ...
  for i = 1:caseCount
    ...
    evaluate(dynamicPower);
  end

  error.expectation = error.expectation / caseCount;
  error.variance = error.variance / caseCount;
  for i = 1:orderCount
    for j = 1:sampleCount
  ...
  function evaluate(dynamicPower)
    output = mc.compute(dynamicPower);

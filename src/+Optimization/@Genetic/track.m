...
function printUniobjective(state, flag)
  ...
  case 'init'
    fprintf('%10s%15s%15s%15s%15s%15s%15s\n', 'Generation', ...
      'Evaluations', 'Cached', 'Penalized', ...
      'New', 'Penalized', 'Best');
  case { 'iter', 'done' }
    fprintf('%10d%15d%15.2f%15.2f%15.2f%15.2f%15.2e\n', ...
      state.Generation, state.FunEval, ...
      state.cachedCount / state.FunEval, ...
      state.penalizedCount / state.FunEval, ...
      state.newCount / size(state.Population, 1), ...
      state.newPenalizedCount / state.newCount, ...
      state.Best(end));
  end
end
...
function printMultiobjective(state, flag)
  ...
  case 'init'
    fprintf('%10s%15s%15s%15s%15s%15s%15s\n', 'Generation', ...
      'Evaluations', 'Cached', 'Penalized', ...
      'New', 'Penalized', 'Non-dominants');
  case { 'iter', 'done' }
    fprintf('%10d%15d%15.2f%15.2f%15.2f%15.2f%15d\n', ...
      state.Generation, state.FunEval, ...
      state.cachedCount / state.FunEval, ...
      state.penalizedCount / state.FunEval, ...
      state.newCount / size(state.Population, 1), ...

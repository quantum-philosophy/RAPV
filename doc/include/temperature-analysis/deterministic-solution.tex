...
The pseudocode of this algorithm is given in \aref{deterministic-solution}.\footnote{Hereafter, we utilize the MATLAB notation of referencing a particular column, \eg, $\m{A}(:, k)$, or a particular row, \eg, $\m{A}(k, :)$, of a matrix $\m{A}$. Also, we shall underline auxiliary vectors, \eg, $\tmp{\v{a}}$, and matrices, \eg, $\tmp{\m{A}}$.}

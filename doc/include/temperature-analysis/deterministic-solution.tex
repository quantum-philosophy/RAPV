...
The pseudo code of this algorithm, which delivers the exact solution under the made assumptions, is given in \aref{deterministic-solution-without-leakage}.
...
\begin{remark}
The time complexity of direct solutions of the system of linear equations is $\complexity{\nsteps^3 \nnodes^3}$ while the one of \aref{deterministic-solution-without-leakage} is only $\complexity{\nsteps \nnodes^2 + \nnodes^3}$.
Note that the complexity of \aref{deterministic-solution-without-leakage} reported in \cite{ukhov2012} was overestimated.
\end{remark}
\input{include/algorithms/deterministic-solution-without-leakage.tex}

...
The mathematical foundation of this spectral approach is outlined in \xref{spectral-decomposition}; here we leap directly to the final result.
...
The compositional function in \eref{spectral-decomposition} is nothing more than a polynomial; hence, it is easy to interpret and easy to evaluate.
...
The primary question now is the calculation of the coefficients $\{ \coefficient{\w}_{\multiindex} \}$, which are the only unknowns in \eref{spectral-decomposition}.

\subsubsection{Spectral Projection}
As shown in \xeref{orthogonal-projection} in \xref{spectral-decomposition}, each $\coefficient{\w}_{\multiindex}$ is an $\nvars$-dimensional integral, and the integral should be taken numerically.

...
It is worth noting that $\w$ can be (and will be in our application) multidimensional.
In such a case, we shall consider $\w$ as a random vector with an appropriate number of elements.
Then such operations as those in \eref{spectral-decomposition} and \eref{numerical-integration} should generally be undertaken elementwise.
Also, it will be convenient to assume that $\w$ is a row vector.
In particular, the projection in \eref{quantity-evaluation} will need no modifications by letting $\coefficient{\vw}$, $\vw$, and $\w_i$ be two matrices and a row vector, respectively.
\end{remark}

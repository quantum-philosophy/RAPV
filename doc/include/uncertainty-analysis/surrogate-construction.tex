...
Once these two components have been identified, and the desired approximation level $\level = \nclevel = \nqlevel$ has been specified, the corresponding polynomial basis and quadrature rule stay the same for all quantities that one might be interested in studying.
...
In particular, the construction of the expansion in \eref{spectral-decomposition} can be reduced one matrix multiplication as follows.
...
More specifically, such operations as those in \eref{spectral-decomposition} and \eref{numerical-integration} should be undertaken elementwise, and, in \eref{quantity-evaluation}, $\coefficient{\vw}$, $\vw$, and $\w_i$ are generally two matrices and a row vector, respectively.
\end{remark}
